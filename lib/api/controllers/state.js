    ...
    if (val && typeof val !== 'object') {
        if (val === 'true' || val === 'false') {
            const obj = await adapter.getForeignObjectAsync(id, {
                user,
                limitToOwnerRights: adapter.config.onlyAllowWhenUserIsOwner,
            });
        ...
        commonLib.checkPermissions(req._adapter, req._user, [{ type: 'state', operation: 'read' }], async error => {
            if (error) {
                ...
                for (let k = 0; k < oId.length; k++) {
                    try {
                        const { state, id, originId } = await new Promise((resolve, reject) =>
                            commonLib.getState(req._adapter, oId[k], req._user, (error, state, id, originId) =>
    ...
    readBinaryState: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{ type: 'state', operation: 'read' }], async error => {
            if (error) {
                ...
                let foundID;
                try {
                    const { binary, id, originId } = await new Promise((resolve, reject) =>
                        commonLib.getBinaryState(req._adapter, oId, req._user, (error, binary, id) =>
                        ...
                        try {
                            ...
                            if (obj && obj.common && obj.common.type === 'file') {
                                if (req._adapter.setForeignBinaryStateAsync) {
                                    await req._adapter.setForeignBinaryStateAsync(id, req.body, {
                                        user: req._user,
                                        limitToOwnerRights: req._adapter.config.onlyAllowWhenUserIsOwner,
                                    });
                                } else {
                                    await req._adapter.setBinaryStateAsync(id, req.body, {
                                        user: req._user,
                                        limitToOwnerRights: req._adapter.config.onlyAllowWhenUserIsOwner,
                                    });
                                }
    ...
    plainState: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{ type: 'state', operation: 'read' }], async error => {
            if (error) {
                ...
                try {
                    const { state, id, originId } = await new Promise((resolve, reject) =>
                        commonLib.getState(req._adapter, oId[0], req._user, (error, state, id, originId) =>
                            error ? reject(error) : resolve({ state, id, originId }),
                        ),
                    );
    ...
    unsubscribeState: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{ type: 'state', operation: 'read' }], async error => {
            if (error) {
                ...
                try {
                    await req._swaggerObject.unregisterSubscribe(
                        url,
                        params.stateId,
                        'state',
                        req._user,
