...
module.exports = {
            ...
            if (err) {
                ...
                try {
                    ...
                    if (!obj) {
                        ...
                        await req._swaggerObject.registerSubscribe(req.body.url, params.objectId, 'object', req._user);
    ...
    unsubscribeSObject: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{type: 'object', operation: 'read'}], async err => {
            if (err) {
                res.status(403).json({error: err});
            } else {
                ...
                try {
                    await req._swaggerObject.unregisterSubscribe(req.body.url, params.objectId, 'object', req._user);
    ...
    subscribeObjects: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{type: 'object', operation: 'read'}], async err => {
            if (err) {
                ...
                try {
                    await req._swaggerObject.registerSubscribe(req.body.url, req.body.pattern, 'object', req._user);
                } catch (error) {
    ...
    unsubscribeObjects: function (req, res) {
        commonLib.checkPermissions(req._adapter, req._user, [{type: 'object', operation: 'read'}], async err => {
            if (err) {
                ...
                try {
                    await req._swaggerObject.unregisterSubscribe(req.body.url, req.body.pattern, 'object', req._user);
                    res.status(200).json({result: 'OK'});
                } catch (error) {
                    res.status(500).json({error, pattern: req.body.pattern, url: req.body.url});
                }
