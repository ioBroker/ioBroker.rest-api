...
function parseQuery(_url) {
    ...
    if (pos !== -1) {
        ...
        for (let i = 0; i < arr.length; i++) {
            arr[i] = arr[i].split('=');
            values[arr[i][0].trim()] =
                arr[i][1] === undefined ? null : decodeURIComponent((arr[i][1] + '').replace(/\+/g, '%20'));
        }
...
function SwaggerUI(_ignore, webSettings, adapter, instanceSettings, app, callback) {
        ...
        if (that.config.auth) {
            ...
            if (!values.user || !values.pass) {
                if (req.headers.authorization?.startsWith('Basic ')) {
                    ...
                    that.adapter.getSession(`a:${accessToken}`, obj => {
                        if (obj) {
                            ...
                            callback = null;
                            that.adapter.log.warn(`Invalid access token: ${values.user}`);
    ...
    // parse binary files
    this.app.post(`${this.routerPrefix}v1/file/*`, multer().fields([{ name: 'file', maxCount: 1 }]), (req, res, next) =>
        next(),
    );
    ...
    if (!this.config.dataSource) {
        this.adapter.getForeignObjectAsync('system.config').then(obj => {
