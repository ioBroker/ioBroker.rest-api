...
function SwaggerUI(_ignore, webSettings, adapter, instanceSettings, app, callback) {
    ...
    this.commands   = new CommandsAdmin(adapter);
    ...
    this.app.use(this.routerPrefix + 'v1/command/*', (req, res) => {
        let command = req.originalUrl.split('/')[3];
                ...
                if (args[args.length - 1] === 'callback') {
                    _arguments.push((error, ...args) => {
                        ...
                        if (args.length === 1) {
                            ...
                            // if file
                            if ((params.binary === null || params.binary === true) && args[1] && typeof args[1] === 'string' && args[1].includes('/') && Buffer.isBuffer(args[0])) {
