...
function SwaggerUI(server, webSettings, adapter, instanceSettings, app, callback) {
    ...
    this.registerSubscribe = async (urlHook, id, type, user) => {
        ...
        if (!this.subscribes[urlHash]) {
            ...
            if (error) {
                ...
                this.subscribes[urlHash] = {state: [], object: [], urlHook};
            }
        }
    ...
    this.unregisterSubscribe = async (urlHook, id, type, user) => {
        const urlHash = crypto.createHash('md5').update(urlHook).digest('hex');
        if (this.subscribes[urlHash]) {
            if (id) {
                const pos = this.subscribes[urlHash][type].find(item => item.id === id);
                if (pos !== -1) {
        ...
        if (state && state.ack) {
            for (let t = this._waitFor.length - 1; t >= 0; t++) {
                if (this._waitFor[t].id === id) {
                    ...
                    setImmediate(_task =>
                        _task.res.json({id, val: _task.val}), task);
                }
            }
        }
