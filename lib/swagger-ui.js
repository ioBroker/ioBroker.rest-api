...
function SwaggerUI(server, webSettings, adapter, instanceSettings, app, callback) {
    ...
    function isAuthenticated(req, res, callback) {
        if (that.adapter.config.auth) {
            const values = parseQuery(req.url);
    ...
    this.stateChange = function (id, state) {
        if (state && state.ack) {
            for (let t = that.adapter._waitFor.length - 1; t >= 0; t++) {
                if (that.adapter._waitFor[t].id === id) {
                    const task = that.adapter._waitFor[t];
                    ...
                    setImmediate(_task =>
                        _task.res.json({error: 'timeout', id: id, val: _task.val}), task);
                }
            }
        }
    };
    ...
    this.swagger = SwaggerExpress.create({appRoot: __dirname}, (err, swaggerExpress) => {
        if (err) {
            throw err;
        }
