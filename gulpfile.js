...
const description = {
    ...
    'getStates': 'get the list of states for pattern (e.g. for system.adapter.admin.0.*). GUI can have problems by visualization of answer.',
    ...
    'getObjectView': 'get specific objects, e.g. design=system, search=state, params={\\"startkey\\": \\"system.adapter.admin.\\", \\"endkey\\": \\"system.adapter.admin.\\u9999\\"}',
    'setObject': 'set object with JSON object (e.g. {\\"common\\": {\\"type\\": \\"boolean\\"}, \\"native\\": {}, \\"type\\": \\"state\\"})',
    ...
    'readFile': 'read file, e.g. adapter=vis.0, fileName=main/vis-views.json. Additionally, you can set option in query binary=true to get answer as file and not as json',
    'readFile64': 'read file as base64 string, e.g. adapter=vis.0, fileName=main/vis-views.json. Additionally, you can set option in query binary=true to get answer as file and not as json',
    ...
    'chownFile': 'change file owner. E.g. adapter=vis.0, fileName=main/*, options = {\\"owner\\": \\"newOwner\\", \\"ownerGroup\\": \\"newgroup\\"}',
    ...
    'getHistory': 'read history. See for options: https://github.com/ioBroker/ioBroker.history/blob/master/docs/en/README.md#access-values-from-javascript-adapter',
...
gulp.task('generateList', done => {
    ...
    const ignore = ['authenticate', 'name', 'error', 'logout', 'eventsThreshold', 'writeFile', 'getEasyMode', 'authEnabled', 'requireLog', 'getIsEasyModeStrict', 'cmdExec', 'sendToHost'];
    ...
    Object.keys(commands.commands).filter(name => !ignore.includes(name) && !name.includes('subscribe')).forEach(func => {
        ...
        args.forEach(arg => {
            ...
            if (func !== 'chmodFile' && func !== 'chownFile' &&
